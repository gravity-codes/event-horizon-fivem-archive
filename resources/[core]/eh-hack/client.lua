...
function StartHack(life, time, returnFn)
...
RegisterCommand('testhack', function(source, args)
	...
	StartHack(lives, time, TestCallback)
end)

TriggerEvent('chat:addSuggestion', '/testhack', 'Starts a hack minigame for the client.',
	{
		{ name = "lives", help = "number of tries {6 lives max}" },
		{ name = "seconds", help = "seconds to solve {40 minimum | 180 maximum}" }
	}
)

function TestCallback(outcome, reason)
	if outcome == 0 then
		Citizen.Trace('Hack failed', reason)
	elseif outcome == 1 then
		Citizen.Trace('Hack successful')
	elseif outcome == 2 then
		Citizen.Trace('Timed out')
	elseif outcome == -1 then
		Citizen.Trace('Error occured', reason)
	end
end